...
%%
\input{tables/techniques-artifacts-capabilities}
...
\begin{figure*}
    \centering
    \includegraphics[width=1\linewidth]{figures/evolution-tracking-techniques.pdf}
...
%%
\subsubsection{Privacy-preserving Paradigms}
\label{sec:mixed-techniques-privacy-paradigms}

%%
\todo{Yohan to finalize this sub-sub-section to ensure it flows with other sections and contains necessary details regarding privacy preserving paradigms such as privacy sandbox, PCM, etc.}
%
Google's Privacy Sandbox replaces cross-site identifiers with on-device interest signals computed from a user's browsing history as the browser maintains stateful history locally but discloses only a coarse signal.
%
Its first iteration, FLoC, assigned users to cohorts that proved unique enough over time to re-identify individuals, especially when combined with fingerprinting signals~\cite{berkePrivacyLimitationsInterestbased2022}.
%
Its successor, the Topics API, discloses coarse interest topics per epoch instead, but 57\% of users with stable interests remain uniquely re-identifiable after 15 epochs, rising to 75\% after 30 once advertiser collusion is assumed~\cite{jhaRobustnessTopicsAPI2023,beuginInterestdisclosingMechanismsAdvertising2024}.
%
Protected Audience moves ad auctions on-device to avoid disclosing interest-group membership entirely, though indirect leakage through auction timing and outcome side-channels remains under active investigation~\cite{calderonioFledgingWillContinue2024,aliNavigatingMurkyWaters2023}.

\begin{opbox}
\todo{Yohan to check the open problem if we want to keep it broadly focused on privacy preserving paradigms in general rather than just interests?} 
...
% %%
% \para{AI as an Emerging Tracking Surface.} 


%%
% As third-party cookies continue to recede across browsers, we expect authentication-based identity graphs, cross-context bridging, and robust privacy-preserving mechanisms to compensate the tracking that cookies used to support, making them, alongside server-side tracking (\autoref{sec:stateful-techniques-serverside}), the clearest candidates for where the field's attention should shift over the next several years.
