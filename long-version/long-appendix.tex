\section{Web Ecosystem Background}
\label{sec:appendix-background}


\subsection{Architecture of the Web}
%%
% The Web is comprised of a client-server architecture built on the HTTP(S) protocol where browsers (client) send HTTP requests to servers---identified by URLs (Uniform Resource Locators) that specify the scheme (protocol), host (domain name), and resource path---sharing requested resources as HTTP responses.
% %
% The web server shares the requested resource as an HTTP response to the user's browser.
%
The Web is comprised of a client-server architecture built on the HTTP(S) protocol.
%
...
%
The actual transmitted data is known as payload.
%
...
%
Thus, external resource inclusion is a powerful web feature that enables the reuse of content and functionality across websites.
%
This also means that multiple parties can be present in the context of a single webpage.


\subsection{Browser's Origin Model}
%
...
%
Modern browsers also group related origins into a broader notion of ``site'' based on the effective top-level domain plus one (eTLD+1)~\cite{TrackingPreventionWebKit2020} using public suffix lists\footnote{e.g., \url{https://publicsuffix.org/}}.
%
...
%%
\subsection{Browser’s Context Model}
%
...
\subsection{Browser-Enforced Policies}
\para{Context-Origin Boundaries.}
%
...
%
Conversely, if the contexts are from different origins and even if they are loaded side by side, the browser’s SOP enforces separation---scripts in one context cannot directly read or modify the content of the other.
%
This means, for example, a page from \texttt{news.com} cannot inspect the DOM of an embedded \texttt{tracker1.com} iframe, nor can \texttt{tracker1.com} script alter the parent page, because their origins don’t match.
%
...
%
Requesting a resource doesn’t violate the SOP because the including page isn’t directly accessing or manipulating the embedded resource’s DOM, state or data; it’s either displaying it or executing it as intended.
%
...
%
The act of inclusion in this case is essentially an implicit trust declaration by the webpage as if it trusts that code with its own origin’s privileges~\cite{TrackingPreventionWebKit2020}.
%
...
%
JavaScript cookies can be accessed (e.g., using \texttt{document.cookie} or CookieStore API) by any script running in the same execution context regardless of its source.
%
...
\input{appendix/need-for-fp.tex}


% \newpage
\input{tables/appendix-tracking-technique-papers-techniques}


% \newpage
\input{tables/appendix-tracking-techniques-papers-artifacts-capabilities}
\input{tables/defense-deployment-mechanism-type}
...
% \newpage
\input{tables/defenses-industrial}
\input{tables/defenses-community}
