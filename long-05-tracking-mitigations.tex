...
%
For industry-based and community-based defenses, we identify \todo{XX} defense techniques against stateful tracking (see \autoref{tab:stateful-tracking-defenses}) and \todo{YY} defense techniques against stateless tracking (see \autoref{tab:stateful-tracking-defenses}). 
%
...
%%
\input{tables/stateful-tracking-defenses}
...
%%
\input{tables/defense-goal-mechanism-taxonomy}
...
%%
\input{tables/stateless-tracking-defenses}
...
\todo{Yohan to review this section and open problems.}
...
\para{Defending Against Cross-device and Cross-context Tracking.}
%
...
%
Browsers instead address the authentication pathway with purpose-built APIs: the Storage Access API, now shipped by all five major browsers, lets a third party request first-party-equivalent storage access explicitly rather than obtaining it ambiently, and Chrome and Edge additionally support Federated Credential Management (FedCM), which preserves single sign-on while preventing the identity provider from learning which relying party the user logs into.
%
Brave blocks unrecognized identity providers by default, treating them the same as any other third-party tracker.
%
On mobile, Apple's App Tracking Transparency (ATT) framework has required explicit user opt-in before an app can access the device's advertising identifier for cross-app tracking since iOS 14.5, and Google's Privacy Sandbox for Android had proposed similar restrictions on the Android advertising ID.
%
% For ultrasonic cross-device tracking, proposals have pushed for OS-level APIs that control microphone access and selectively suppress inaudible beacon frequencies~\cite{mavroudisPrivacySecurityUltrasound2017}.
%
However, WebViews and in-app browsers---which render a growing share of web traffic---share none of the standalone browser's tracking defenses: no extension support, no storage partitioning, and no filter-list blocking, leaving users exposed within these embedded contexts.
...
\para{Privacy-preserving Advertising APIs.}
Google's Privacy Sandbox was the most extensive industrial attempt to replace cross-site identifiers with on-device computation: FLoC, launched in an origin trial in 2021, was withdrawn within a year over re-identification concerns and replaced by the Topics API, and FLEDGE (later renamed Protected Audience) moved ad auctions on-device to avoid disclosing interest-group membership to the seller at all.
%
Apple shipped Private Click Measurement (PCM) in 2021 as a narrower, privacy-preserving alternative for ad-click attribution alone.
%
By late 2025, Google had retired most Privacy Sandbox APIs while leaving third-party cookies in place in Chrome, leaving PCM as the more durable of the two vendors' proposals; \autoref{sec:mixed-techniques-privacy-paradigms} discusses why these interest-disclosing designs kept running into the same re-identification limits regardless of vendor.
%
All major browser vendors continue to coordinate this work jointly through the W3C's Private Advertising Technology Community Group (PATCG), chartered in 2021, rather than shipping unilateral, mutually incompatible proposals.

\begin{opbox}
Privacy Sandbox demonstrated that a tracking-relevant signal does not need to leave the device to remain useful to an advertiser, yet every browser vendor built and then abandoned its own incompatible version of this idea. What would a standardized, cross-browser on-device computation primitive look like that advertisers could build against regardless of which browser a user runs, without requiring any single vendor to unilaterally set the privacy bar for the rest of the web?
\end{opbox}
...
%
Because cross-device and authentication-based tracking so often runs through a shared identity provider, several proposals harden that layer directly rather than the storage or fingerprint underneath it.
%
SPRESSO inserts a forwarder between the relying party and the identity provider so the provider itself cannot learn which site a user is logging into~\cite{fettSPRESSOSecurePrivacyRespecting2015}, and encrypted access logging lets a service prove and audit device access without retaining a persistent device identifier of its own~\cite{ortegaperezEncryptedAccessLogging2025}.
%
At a broader scale, identity-system reform proposals re-architect how national and platform-level identity infrastructure is built so that using the same identity across services does not, by default, let those services correlate a user's activity~\cite{brandaoMendingTwoNationScale2015}.


%%
\para{Cryptographic Advertising Proposals.}
%
A parallel line of academic proposals shows that the specific measurements advertisers need, such as conversion attribution and bid optimization, can be computed without either party learning an individual user's data.
%
Adnostic, Privad, and ObliviAd were among the earliest to demonstrate this for on-device ad selection~\cite{toubianaAdnosticPrivacyPreserving2010,guhaPrivadPracticalPrivacy2011,backesObliviAdProvablySecure2012}, and Ibex later showed the same guarantee holds for conversion attribution and bid optimization jointly, using cryptographic aggregation rather than the coarse, epoch-based disclosure that Topics and Protected Audience relied on~\cite{zhongIbexPrivacypreservingAd2022}.
%
Few of these cryptographic designs have been adopted directly by a browser vendor, largely because they demand more computation or coordination than an on-device heuristic, but they remain the clearest evidence that interest-based advertising does not inherently require disclosing anything about an individual user at all.
...
Do Not Track (DNT), proposed in 2009 as a browser header declaring a user's tracking preference, was adopted by every major browser within a few years but was never given legal force, and the W3C working group tasked with standardizing it closed in 2019 without industry adoption of a common interpretation.
%
Global Privacy Control (GPC), led by EFF and a coalition of publishers and browsers since 2020, is widely seen as DNT's successor and succeeds where DNT failed by being legally enforceable: California's 2022 settlement with Sephora established that ignoring a GPC signal violates the CCPA, and Colorado has since imposed a similar requirement, driving broader site-side adoption than DNT ever achieved.
%
DuckDuckGo, Brave, and Firefox send GPC by default; Chrome added support only in 2026, years after smaller vendors~\cite{googleGlobalPrivacyControl2025}.
%
Because GPC and DNT are not tied to any single storage or fingerprinting mechanism, they act as a cross-cutting signal that, in principle, covers every technique discussed in this SoK at once, provided a site chooses to honor it.